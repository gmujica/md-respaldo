...
function ViewModel() {
    ...
    this.lstActividad = new GridModelSP("configuracion-listado-actividad-economica", 0, 10);
    ...
    this.addNew = function () {
        ...
        PostJson("configuracion-new-actividad-economica", model).done(function (resultado) {

            if (resultado.success == true) {
                self.init();
                $("#ficha_add").addClass('hidden');
                toastr.success('Actividad Economica Registrado Correctamente');
                self.clear();
            } else {
    ...
    this.editar = function () {
        ...
        PostJson("configuracion-edit-actividad-economica", model).done(function (resultado) {

            if (resultado.success == true) {
                self.init();
                $("#ficha_edit").addClass('hidden');
                toastr.info('Actividad Economica actualizada Correctamente');
            } else {
                toastr.error(resultado.error);
                return false;
            }
    ...
    this.eliminar = function () {
        ...
        console.log(model);
        PostJson("configuracion-eliminar-actividad-economica", model).done(function (resultado) {

            if (resultado.success == true) {
                self.init();
                $("#modal_remove").addClass('hidden');
                toastr.info('Actividad Economica eliminado Correctamente');
            } else {
                toastr.error(resultado.error);
                return false;
            }
